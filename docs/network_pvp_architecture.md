...
状态核对：2026-09-16。本文维护 **PVP 产品方向与长期权威结算设计**，
不是已上线功能列表，也不改变当前 Web 优先顺序。三层模式与现有代码状态见
[混合架构](hybrid_pvp_architecture.md)，端点与传输格式见
[Worker 指南](../scripts/server/cf_worker/README.md)。
文中的服务器、Nakama、正式积分结算及未决产品规则属于目标方案。
...
2026-09-16 静态核对：双方显式发射、双输入和帧计数已存在；Godot 有快照恢复、
编码与哈希模块，战斗页也已接入 BattleSession。不能再把它们全部列为“尚未开始”。

剩余差异与验收缺口：

1. Web 新场景求解器与 Godot / 网络声明版本不同，见 [版本边界](deterministic_battle_sync.md)。
2. Web 主游戏未接入 BattleSession，Web 求解器尚无快照恢复接口。
3. 独立回放播放器、命令行权威验算器及完整跨端一致性验收仍缺少交付依据。
4. Worker 当前接收回放并存入 R2，`accepted` 不等于战斗结果通过权威验算。
5. 本地库存、金币、奖励与可调参数不能直接作为正式 PVP 的服务端结算依据。
...
以下对象是目标数据模型，不是当前 WebSocket 字节格式。
实际网络层已使用二进制协议 v2；HTTP API 仍使用 JSON。
实现字段与量化以三端 `protocol` 源文件为准，不能按这里的概念字段直接发包。
...
本节是 2026-07 方案背景，所列价格、配额、产品可用性和容量未在本次整理中联网核实。
部署前按下列官方链接及实际账号计划重查，不将这些数字作为当前保证。
技术封测的帧同步中继见混合架构；正式排位仍以服务端权威结果为目标。
...
5. Cloudflare 可作为技术封测中继候选；中国正式排位的路由与节点需经目标移动网络压测，
   不根据“免费层可用”推导实时质量已达标。
