# Godot AI / MCP 候选方案（待核实）

状态：2026-09-16 标记为可选背景资料。未锁定具体插件仓库、发布版本或兼容矩阵，
下文的产品名称和 UI 步骤不是已验证安装教程。Web 开发不需要先配置 Godot MCP。
接手入口见 [文档导航](README.md)。

恢复为可执行安装指南前，应补齐：官方仓库 URL、固定版本、支持的 Godot 版本、
真实服务启动命令及一次连接验证记录。不要按通用名称猜测 npm 包。

本文档用于说明如何让 AI 助手更好地操作 Godot 项目。目标是让 AI 可以读取 Godot 编辑器中的场景、节点、脚本、资源和错误信息，而不只是修改磁盘上的文本文件。

## 先选哪种方案

原候选名称（没有足够证据给出当前推荐顺序）：
...
安装前按实际仓库确认维护状态、授权和费用；这些名称可能对应不同项目。
...
不同 AI 客户端和插件配置方式不同。旧示例的 `godot-mcp-server` 没有对应的
已核实仓库与版本，已撤下可执行配置。应从最终选定插件的官方文档生成配置，
并记录连接验证；不能把一个同名包当成项目已采用的服务。
